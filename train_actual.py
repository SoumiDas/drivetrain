...
WEIGHTS = {'red_light': torch.Tensor([0.11, 2.0000]),
           'hazard_stop': torch.Tensor([0.026, 2.0000]),
           'speed_sign': torch.Tensor([0.02, 0.8945283 , 1.82241015, 2.00]),
...
def loss_batch(model, loss_func, preds, labels, opt=None):
    loss, lossr, lossc, lossred, lossh, losssp, lossv = loss_func(preds, labels)
...
def fit(epochs, model, params, loss_func, opt, train_dl, valid_dl, dev='cuda', val_hist=None, val_hist_total=None, maper=None):
    ...
    for epoch in range(epochs):
        ...
        #tr_loss, modified_trloss, trpreds, trlabels = validate(model, train_dl, loss_func)
